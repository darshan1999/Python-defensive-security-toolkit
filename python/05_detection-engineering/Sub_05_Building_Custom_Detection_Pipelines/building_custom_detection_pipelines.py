...
#         ↓
#     Alerting / Output
# - Decoupling: Each stage is a function or class with clear input/output.
# - Batching: Process records in batches for efficiency.
# - Error Handling: Failures in enrichment or detection should not drop entire batch.
# - Configuration: Rules, thresholds, API keys live in external YAML/JSON.
#
import json
import yaml
import requests
...
# - Stateful Detection: For brute-force, maintain counters in external store (Redis,
#   DB).
# - Batching & Parallelism: Use Python’s `concurrent.futures` or message queues for high
#   throughput.